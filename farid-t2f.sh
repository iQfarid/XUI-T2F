...
install_go() {
    ...
    local URLS=(
        "https://dl.google.com/go/go${REQUIRED}.linux-${GO_ARCH}.tar.gz"
        "https://go.dev/dl/go${REQUIRED}.linux-${GO_ARCH}.tar.gz"
        "https://mirror.nju.edu.cn/golang/go${REQUIRED}.linux-${GO_ARCH}.tar.gz"
        "https://golang.dl.tencentcloudapi.com/go${REQUIRED}.linux-${GO_ARCH}.tar.gz"
    )
    ...
    local DOWNLOADED=0
    for URL in "${URLS[@]}"; do
        echo -e "${C_YELLOW}تلاش برای دانلود از: $(echo "$URL" | cut -d/ -f3)...${C_RESET}"
        if curl -sL --connect-timeout 10 --max-time 30 --retry 2 -o "$TMP" "$URL" || wget -q --timeout=30 -O "$TMP" "$URL"; then
            if file "$TMP" 2>/dev/null | grep -qi "gzip" || gzip -t "$TMP" 2>/dev/null; then
                DOWNLOADED=1
                break
            fi
        fi
    ...
    if [ "$DOWNLOADED" -ne 1 ]; then
        error_exit "امکان دانلود فایل معتبر Go وجود ندارد. سرور شما به هیچ‌کدام از لینک‌ها یا آینه‌ها دسترسی ندارد."
    fi

    rm -rf /usr/local/go
    tar -C /usr/local -xzf "$TMP" || error_exit "استخراج پکیج Go با خطا مواجه شد."
    rm -f "$TMP"
...
build_panel() {
    ...
    local FIRST_BUILD_MARKER="$BASE_DIR/.build_done_once"
    ...
    for ATTEMPT in 1 2; do
        ...
        if [ "$ATTEMPT" -eq 2 ]; then
            echo -e "${C_YELLOW}⚠ تلاش اول ناموفق بود؛ در حال پاکسازی کش ماژول‌ها و تلاش مجدد...${C_RESET}"
        ...
        if [ ! -f "$FIRST_BUILD_MARKER" ]; then
            ...
            # ----------------------------------------------------
            # بار اول: توضیح کامل دلیل کندی + نمایش زنده‌ی پیشرفت
            # ----------------------------------------------------

            echo -e "${C_YELLOW}این اولین باره که پنل روی این سرور Build می‌شه.${C_RESET}"
            echo -e "${C_CYAN}به همین دلیل ممکنه ۱۰ تا ۱۵ دقیقه طول بکشه، چون:${C_RESET}"
            echo "    • در صورت نبود Go، باید دانلود و نصب بشه"
            echo "    • ماژول‌های وابسته (به‌خصوص xray-core که حجیمه) باید دانلود بشن"
            echo "    • کل پروژه برای اولین بار کامپایل می‌شه"
            echo
            echo -e "${C_GRAY}(دفعات بعد که ضریب رو عوض کنی، این مرحله فقط چند ثانیه طول می‌کشه چون کش نگه داشته می‌شه)${C_RESET}"
            echo

            go build -v -o "$NEW" . >"$LOG" 2>&1 &
            local BUILD_PID=$!
            local START_TS
            START_TS="$(date +%s)"
            ...
            while kill -0 "$BUILD_PID" 2>/dev/null; do
                ...
                printf "\r\033[K  ⏳ %02d:%02d — در حال کامپایل: %s" "$((ELAPSED / 60))" "$((ELAPSED % 60))" "${LAST_LINE:0:55}"
                sleep 1
            done
            ...
            # ----------------------------------------------------
            # بار بعد: پیام کوتاه (کش موجوده، معمولاً سریع تمام می‌شه)
            # ----------------------------------------------------

            echo -e "${C_YELLOW}در حال Build پنل...${C_RESET}"
            echo -e "${C_CYAN}(ممکنه کمی طول بکشد)${C_RESET}"

            go build -o "$NEW" . >"$LOG" 2>&1 || BUILD_STATUS=$?
